function doFlips() {
    ...
    var numLoops = 100;
    //for (var loopNum = 1; loopNum < numLoops; loopNum++) {

    var probability = 0.75;

    var results = [];

    for (var percentageBet = 0; percentageBet <= 100; percentageBet++) {

        var currentMoney = 100;
        var numFlips = 100;
        for (var flipNum = 0; flipNum < numFlips; flipNum++) {
            var randNum = Math.random();
            if (randNum < probability) {
                currentMoney = currentMoney * (1 + percentageBet / 100);
            } else {
                currentMoney = currentMoney * (1 - percentageBet / 100);
            }
        }
        results[percentageBet] = currentMoney;
